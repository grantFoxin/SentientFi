...
import { getPortfolioCheckQueue, getRebalanceQueue } from "../queues.js";
...
export async function processPortfolioCheckJob(
  ...
  if (!marketCheck.safe) {
    logger.warn(
      "[WORKER:portfolio-check] Market conditions unsafe, skipping cycle",
      {
  ...
  const portfolioQueue = getPortfolioCheckQueue();

  if (!portfolioQueue) {
    logger.error("[WORKER:portfolio-check] Queue unavailable, skipping cycle");
    return;
  }
  const client = await portfolioQueue.client;
  ...
  for (const portfolio of allPortfolios) {
    try {
      ...
      // Get the portfolio data snapshot up front to evaluate cooldowns
      ...
      const needsRebalance = await stellarService.checkRebalanceNeeded(
        portfolio.id,
      );
      if (!needsRebalance) {
        skipped++;
      ...
      // Fix Root Cause 3: Pre-flight check for Daily Cap Counter
      const todayStr = new Date().toISOString().split("T")[0]; // Format: YYYY-MM-DD
      const counterKey = `auto-rebalance-count:${portfolio.id}:${todayStr}`;

      const rawCount = await client.get(counterKey);
      const dailyCount = parseInt(rawCount || "0", 10);

      if (dailyCount >= 3) {
        // MAX_AUTO_REBALANCES_PER_DAY
        logger.warn(
          "[WORKER:portfolio-check] Rebalance blocked: Daily budget exhausted",
          {
      ...
      if (!riskCheck.allowed) {
        logger.warn(
          "[WORKER:portfolio-check] Rebalance blocked by risk management",
          {
            portfolioId: portfolio.id,
      ...
      // Concentration risk
      const concentrationCheck = CircuitBreakers.checkConcentrationRisk(
        p.allocations,
      );
      ...
      if (rebalanceQueue) {
        // Fix Root Cause 1: Use a deterministic execution jobId matching the portfolio target instance.
        await rebalanceQueue.add(
          `rebalance-${portfolio.id}`,
          { portfolioId: portfolio.id, triggeredBy: "auto" },
          {
            jobId: `rebalance-exec-${portfolio.id}`,
        ...
        queued++;
        logger.info(
          "[WORKER:portfolio-check] Enqueued unique rebalance execution task",
          {
            portfolioId: portfolio.id,
          },
        );
